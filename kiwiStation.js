...
if(location.href.search('2021_beta') > -1 && location.href.search('kiwi') > -1){
	...
	KSMD.style.backgroundImage = 'url(\'https://maohupi.riarock.com/web/tool/kiwiStation/img/md.png\')';
	...
	for(let i = 0; i < 6; i++){
		...
		KSBtn[i].style.backgroundImage = `url('https://maohupi.riarock.com/web/tool/kiwiStation/img/btn${i+1}.png')`;
	...
	KSScript.innerHTML = `
			...
			xmlhttp.onreadystatechange=function() {
				if (xmlhttp.readyState==4) {
					if(xmlhttp.responseText.search('ERROR') == -1 && xmlhttp.responseText.search('WARNING') == -1 && xmlhttp.responseText.search('NOTICE') == -1){
						alert('送出成功！');
						KS_sendB_times++;
						document.querySelector('[btnId="btn4"]').style.backgroundImage = 'none';
						document.querySelector('[btnId="btn4"]').style.backgroundColor = '#ffffff';
						document.querySelector('[btnId="btn4"]').innerText = 'OK';
					};
				}
			}
			xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		...
		function KS_XMLhttpRobotC(e){
			if(e){
				document.querySelector('[btnId="btn4"]').setAttribute('xmlRobot', 'true');
			}
			else{
				document.querySelector('[btnId="btn4"]').setAttribute('xmlRobot', 'false');
			}
		}
		KS_reSet_usersSelect();
		function KS_addUser(){
